...
# Define a flag to indicate whether the process is running
...
def update_display_periodically(output_lines):
    global process_running
    while process_running:
        if output_lines:
            # Clear the display before showing the latest message
            disp.LCD_Clear()
            # Display the latest output
            display_message_with_wrap(output_lines[-1])  # Display the latest match
        time.sleep(5)  # Update every 5 seconds

def read_output_from_process(process, output_lines):
    while True:
...
def build_and_run_command():
    global process_running
    output_lines = []  # Initialize a buffer for scrolling output
    ...
    if any(option["name"] == "Show Cracked" and option["state"] for option in options):
        ...
        # Start the process and display update thread
        try:
            process_running = True  # Set the process running flag
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

            # Start the output reading thread
            output_thread = threading.Thread(target=read_output_from_process, args=(process, output_lines))
            output_thread.start()

            # Start the display update thread
            display_thread = threading.Thread(target=update_display_periodically, args=(output_lines,))
            display_thread.start()

            process.wait()  # Wait for the process to complete
            process_running = False  # Reset the process running flag

            output_thread.join()  # Ensure output is read completely
            display_thread.join()  # Ensure display updates are complete
